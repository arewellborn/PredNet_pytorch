...
class SequenceGenerator(data.Dataset):
    ...
    def __init__(
        ...
        if (
            ...
            self.possible_starts = np.array(
                [
                    ...
                    if self.sources[i] == self.sources[i + self.step * self.num_timeSteps]
                ]
            )
        elif (
            ...
            while curr_location < self.num_samples - self.step * self.num_timeSteps + 1:
                if (
                    ...
                    == self.sources[curr_location + self.step * self.num_timeSteps]
                ):
        ...
        if (
            N_seq is not None and len(self.possible_starts) > N_seq
...
class ZcrDataLoader(object):
    ...
    def __init__(
        self, data_file, source_file, output_mode, sequence_start_mode, N_seq, args
    ):
    ...
    def dataLoader(self):
        image_dataset = SequenceGenerator(
            self.data_file,
            self.source_file,
