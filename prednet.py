...
def batch_flatten(x):
    ...
    dim = int(
        dim
    )  # 不加这步的话, dim是<class 'numpy.int64'>类型, 不能在view中用. 加上这步转成<class 'int'>类型.
    return x.view(-1, dim)
...
class PredNet(nn.Module):
    ...
    def __init__(
        ...
        if self.output_mode in layer_output_modes:
            ...
            self.output_layer_NO = int(
                self.output_mode[-1]
            )  # suppose the number of layers is < 10
        else:
    ...
    def get_initial_states(self, input_shape):
        ...
        init_height = input_shape[
            self.row_axis
        ]  # equal to `init_nb_rows` in original version
        ...
        states_to_pass = [
            ...
            "c",
            "E",
        ]  # R is `representation`, c is Cell state in LSTM, E is `error`.
        ...
        if self.extrap_start_time is not None:
            initial_states += [
                Variable(torch.IntTensor(1).zero_().cuda())
    ...
    def step(self, A, states, prior_information_model):
        ...
        for lay in reversed(range(self.num_layers)):
            inputs = [
                R_current[lay],
                E_current[lay],
            ]  # 如果是顶层, R_l的输入只有两个: E_l^t, R_l^(t-1). 即没有高层的R模块的输入项.
            if self.isNotTopestLayer(
                lay
            ):  # 如果不是顶层,R_l的输入就有三个: E_l^t, R_l^(t-1), R_(l+1)^t. R_up即为R_(l+1)^t
                inputs.append(R_up)
        ...
        for lay in range(self.num_layers):
            if lay != 0:
                ...
                Ahat[
                    ...
                    self.pixel_max
                )  # passed through a saturating non-linearity set at the maximum pixel value
            ...
            if self.error_activation.lower() == "relu":
                ...
                raise (
                    RuntimeError(
                        "cannot obtain the activation function named %s"
        ...
        if self.output_layer_type is None:
            if self.output_mode == "prediction":
                ...
                for lay in range(self.num_layers):
                    layer_error = torch.mean(
                        batch_flatten(E_list[lay]), dim=-1, keepdim=True
                    )  # batch_flatten函数是zcr依照Kears中同名函数实现的. 第0维是batch_size维度, 将除此维度之外的维度拉平
                    all_error = (
                        layer_error
                        if lay == 0
                        else torch.cat((all_error, layer_error), dim=-1)
                    )
                if self.output_mode == "error":
                    output = all_error
                else:
                    output = torch.cat(
                        (batch_flatten(frame_prediction), all_error), dim=-1
                    )
