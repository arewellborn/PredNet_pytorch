...
pred_weights = weights_f["model_weights"]["pred_net_1"][
    "pred_net_1"
]  # contains 23 item: 4x4(i,f,c,o for 4 layers) + 4(Ahat for 4 layers) + 3(A for 4 layers)
...
pytorch_modules_1 = [
    m + "." + str(2 * i) + "." + item
    for m in pytorch_modules_1
    for i in range(4)
    for item in pytorch_items
]
pytorch_modules_1.remove("A.6.weight")
pytorch_modules_1.remove("A.6.bias")
pytorch_modules_2 = [
    m + "." + str(i) + "." + item
    for m in pytorch_modules_2
    for i in range(4)
    for item in pytorch_items
]
...
for i in range(len(keras_modules)):
    ...
    # weight_dict[pytorch_modules[i * 2 + 1]] = pred_weights[keras_modules[i]]['bias:0']
    weight_dict[pytorch_modules[i * 2]] = np.transpose(
        pred_weights[keras_modules[i]]["kernel:0"][:], (3, 2, 1, 0)
    )
